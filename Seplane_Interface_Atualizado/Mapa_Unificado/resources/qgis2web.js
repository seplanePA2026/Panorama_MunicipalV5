...
map.getView().fit([-4269006.579105, -1091169.182867, -4204286.697957, -1017471.540852], map.getSize());
...
function onSingleClickFeatures(evt) {
    ...
    // Collect clicked points from the education layer (even if multiple overlap)
    var ensinoPoints = [];
    var popupText = '<ul>';
    ...
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
        if (layer && feature instanceof ol.Feature && (layer.get("interactive") || layer.get("interactive") === undefined)) {
            ...
            // Intercept education points: build an enriched card instead of the default table
            if (layer.get('popuplayertitle') === 'ESTAB_DE_ENSINO_PA') {
                var props = (feature.getProperties && feature.getProperties()) ? feature.getProperties() : {};
                var lat = parseFloat(props.LATITUDE || props.latitude || props.lat);
                var lon = parseFloat(props.LONGITUDE || props.longitude || props.lon || props.lng);

                // Fallback: geometry -> lon/lat
                if ((isNaN(lat) || isNaN(lon)) && feature.getGeometry) {
                    try {
                        var ll = ol.proj.toLonLat(feature.getGeometry().getCoordinates());
                        lon = ll[0];
                        lat = ll[1];
                    } catch (e) {}
                }
                if (!isNaN(lat) && !isNaN(lon)) {
                    ensinoPoints.push({ lat: lat, lon: lon });
                }
                return;
            }

            // Default popup behavior for other layers (original qgis2web)
            var doPopup = false;
    ...
    // Close/normalize the default popup section
    var otherHtml = popupText;
    if (otherHtml === '<ul>') {
        otherHtml = '';
    } else {
        otherHtml += '</ul>';
    ...
    // Build enriched section (if available)
    var enrichHtml = '';
    if (ensinoPoints.length && window.q2wBuildEnrichSection) {
        enrichHtml = window.q2wBuildEnrichSection(ensinoPoints);
    }

    var finalHtml = enrichHtml + otherHtml;

    popupContent = finalHtml;
    popupCoord = coord;
    updatePopup();

    // Trigger async enrichment after the popup is in the DOM
    if (ensinoPoints.length && window.q2wEnrichAll) {
        setTimeout(function() { window.q2wEnrichAll(); }, 0);
    }
}
